...
% for M07_07: time_ecg = time_ecg(1:1.55e7,:);
% for M08_08: time_ecg = time_ecg(4000:end,:);
% for M08_08: time_ecg = time_ecg(1:3.13e6,:);
...
desc = 'full'
% filename = 'QRS_detected_1_to_4_2e6.mat'
save(filename,'qrs_times1','t1','rejected_beats1','srate','qrs_amp1','desc')
...
siglab={'y13'}; %Labels for the time-series
...
%%
load('QRS_detected_1_to_4_2e6.mat', 'qrs_times1')
Q_1 = qrs_times1;
load('QRS_detected_7_9e6_to_1_5e7.mat', 'qrs_times1')
Q_2 = qrs_times1;


gap1 = round((7.9e6-4.2e6)/256)
s_gap1 = nan(gap1,1); %Between parts 2 and 1


Q_f = cat(1,Q_1,s_gap1,Q_2);
Yf = obtain_hrv(Q_f);
Yf(Yf>2) = 2;
x = 1:size(Yf);
[HRV,TF,lower,upper,center] = filloutliers(Yf,'linear','percentiles',[0.1 99.9]);
HRV = fill_gaps(Yf);

% gapNan = nan(floor((4.3e4/256)),1);
% QfN = cat(1,gapNan,HRV);
